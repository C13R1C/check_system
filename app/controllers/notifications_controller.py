...
def feed_notifications():
    ...
    return jsonify({
        "notifications": [
            notification_to_dict(n, unread_count=unread_count)
...
def mark_read(notif_id: int):
    ...
    if is_persistent:
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return jsonify({
                "ok": False,
                "error": "La notificación permanece activa hasta resolver el perfil pendiente.",
            }), 400
        flash(
            "La notificación permanece activa hasta resolver el perfil pendiente.",
            "warning",
        )
        return redirect(url_for("notifications.list_notifications"))
...
def mark_all_read():
    try:
        ...
        db.session.rollback()
        return jsonify({"ok": False, "error": "No se pudieron marcar las notificaciones."}), 500
...
def clear_read():
    try:
        ...
        db.session.rollback()
        return jsonify({"ok": False, "error": "No se pudieron limpiar las notificaciones."}), 500
